...
for m in range(sample):
    ...
    #配列
    ele = 2*n+1
    call = np.zeros((n+1, ele)) #リスト何個、リストの要素何個
    ...
    call_bs = S*math.exp(q*t-q*tend)*norm.cdf(x=d1, loc=0, scale=1)-K*math.exp(r*t-r*tend)*norm.cdf(x=d2, loc=0, scale=1)

    listing=np.append(listing, call_model-call_bs)
"""
...
ax=fig.subplots()
#ax.set_xlabel("diff(yen)")
#ax.set_ylabel("count")
#ax.set_title('r=' + str(r) + ', K=' + str(K) + ',sigma=' + str(sig) + ',T=' + str(tend)  + ',q=' + str(q)  + ',n=' + str(n) + ',loop=' + str(sample))
#plt.hist(listing, range=(-50,50), bins=50)

plt.plot(x1, y1, label="call_bs")
plt.plot(x2, y2, label="call_model")
